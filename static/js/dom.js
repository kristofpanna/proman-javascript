...
export let dom = {
    ...
    addToggleListeners() {
        let toggleButton = document.getElementsByClassName("board-toggle");
        for (const button of toggleButton) {
            button.addEventListener('click', dom.toggleCol);
        }
    },

    toggleCol: function (event) {
        let button = event.currentTarget;
        let boardCol = button.parentElement.nextElementSibling;
        if (boardCol.style.display === "") {
            dom.hideCol(boardCol);
        } else {
            dom.showCol(boardCol);
        }
    },

    hideCol: function (boardCol) {
        boardCol.style.display = "none";
    },

    showCol: function (boardCol) {
        boardCol.style.display = "";
    },
